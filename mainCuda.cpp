...
public:
    ...
    __device__ void LoadFile(string filename)
	{
    ...
    __device__ vector<long double> CalcAAndDot(string body_id, vector<long double> r, vector<long double> v)
	{
		a.clear();
		a_dot.clear();
		for (i = 0; i < 3; i++)
    ...
    __device__ void InitiliseTimestep()
	{
		timestep = 999999;
		for (std::pair<std::string, vector<long double> > body_itterator : bodies)
		{
